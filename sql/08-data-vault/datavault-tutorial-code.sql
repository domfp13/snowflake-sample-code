...
//Note: We are creating two new virtual warehouses but you can create as many virtual warehouses as, 
...
//Create a new Snowflake database and schemas to be used in the tutorial
...
--load data time stamp (ldts) and file row number
...
//No records have yet been entered so the query produces no results
...
//We'll be producing sample data by unloading a subset of data from the TPCH sample dataset and then using Snowpipe to load it back 
...
--database in your object explorer.  For this tutorial, we are going to use a subset of objects from the TPC-H set, representing 
--customer and their order.  We are also going to take some reference data about nations and regions.


//Create two stages, one for each data class type - order and customer data.  
//Note: In a production environment, these would likely be internal or external stages.  
...
--and subsets of rows for the customer data and then offload it into the CUSTOMER_DATA stage.

//ORDER data would be extracted into compressed CSV files.

//There are many additional options in the COPY INTO stage construct but we are using INCLUDE_QUERY_ID 
...
//Using a relatively small number of records (i.e., LIMIT) as an example in this tutorial
...
//Set up Snowpipe to load data from files in a stage into staging tables.
...
--In a production environment, you'll likely enable AUTO_INGEST, connecting it with your 
...
--This is because we haven't processed or consumed the delta of that stream yet.
...
--Another thing to notice here is the use of SHA1_binary as a hashing function.  
...
--of functions that use fewer bytes to encode value than STRING.

SELECT * FROM STG_CUSTOMER_STRM SRC;
...
// Check the order stream
SELECT * FROM STG_ORDER_STRM SRC;
...
//We've built out a staging / inbound pipeline, ready to accommodate streaming data and 
...
--there are new records coming in a stream, and that delta will be incrementally propagated to all 
...
--pre-defined frequency (every 1 minute in our example) and use a dedicated virtual warehouse 
--as a compute power (in our tutorial we are going to use the same warehouse for all tasks, 
...
//Create a task for ORDER Stream
...
//let's have a look at the task execution history to see how the process will start.
SELECT *
  FROM TABLE(INFORMATION_SCHEMA.TASK_HISTORY())
  ORDER BY SCHEDULED_TIME DESC;

//Notice how after successful execution, the next two tasks run were automatically SKIPPED as there was nothing in the stream and there is nothing to do.

//We can also check the content and stats of the objects involved.  
...
//One of the great benefits of having the compute power from Snowflake is that now it is totally possible to have most of your business vault and information marts in a Data Vault architecture built exclusively from views. There is no longer a need to have the argument that there are "too many joins" or that the response won't be fast enough. The elasticity of the Snowflake virtual warehouses combined with our dynamic optimization engine has solved that problem. (For more details, see this post)

//If you really want to deliver data to the business users and data scientists in NRT, in our opinion using views is the only option. Once you have the streaming loads built to feed your Data Vault, the fastest way to make that data visible downstream will be viewed. Using views allows you to deliver the data faster by eliminating any latency that would be incurred by having additional ELT processes between the Data Vault and the data consumers downstream.
...
--customer descriptive data could happen in the Business Data Vault, connecting data received from 
...
//Now let's imagine we have a heavier transformation to perform that would make more sense 
...
//What we are going to do from a processing/orchestration perspective is extend our 
...
//Data is automatically flowing through all the layers via asynchronous tasks.  
...
--but we may change the format to simplify users to access and work with the 
--data products/output interfaces. Different consumers may have different needs and preferences,
--some would prefer star/snowflake dimensional schemas, and some would adhere to the use 
...
--is creating views that show the latest version for each key.
--------------------------------------------------------------------
-- RDV curr views
--------------------------------------------------------------------
USE SCHEMA L10_RDV;
...
//All good so far? 
//Now let's try to query fct_customer_order.  You may find that the view does not return any rows. Why? 
--If you remember, when we were unloading sample data, we took a subset of random orders and a subset of random customers. 
--Thus, it is possible that there won't be any overlap, doing the inner join with dim1_order will likely result
...
--customer/reference feed was incomplete. Let's go and see if we can resolve this.
USE SCHEMA L00_STG;
COPY INTO @CUSTOMER_DATA 
FROM
    (SELECT OBJECT_CONSTRUCT(*)    
...
--new customer data into Raw Data Vault. A quick check for the records count in the customer dimension now shows 
...
//Finally let's wear the user's hat and run a query to break down ORDER by nation, region, and ORDER_PRIORITY_BUCKET which
-- are all attributes we derived in Business Data Vault. As we are using Snowsight, why not quickly create a chart
--from this result set to better understand the data. For this simply click on the ‘Chart' section on the bottom pane
--and put attributes/measures as it is shown in the screenshot below.
...
//Cleanup
USE WAREHOUSE COMPUTE_WH;
DROP DATABASE IF EXISTS DV_TUTORIAL;
DROP WAREHOUSE IF EXISTS DV_GENERIC_WH;
DROP WAREHOUSE IF EXISTS DV_RDV_WH;
